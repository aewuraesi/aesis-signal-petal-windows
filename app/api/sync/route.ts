...
import { isWorkspaceEnvelope } from "../../workspace-crypto";

export const dynamic = "force-dynamic";

const unauthorised = () =>
  Response.json(
    { error: "Sign in with ChatGPT to use cloud sync." },
    { status: 401 },
  );
...
export async function GET() {
  ...
  const [row] = await getDb()
    .select()
    .from(accountData)
    .where(eq(accountData.userId, user.userId))
    .limit(1);
  ...
  try {
    ...
    return Response.json(
      { error: "Cloud data is unreadable." },
      { status: 500 },
    );
  }
}
...
export async function PUT(request: Request) {
  ...
  if (
    !body ||
    (!isValidPayload(body.payload) && !isWorkspaceEnvelope(body.payload))
  )
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: accountData.userId,
      set: { payload: JSON.stringify(body.payload), revision, updatedAt: now },
    });
