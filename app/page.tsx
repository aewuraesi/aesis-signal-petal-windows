...
type MetricFocus = "default" | "mine-open" | "mine-overdue" | "mine-resolved" | "mine-total" | "attention-overdue" | "attention-oldest" | "attention-owners" | "attention-first";
type Issue = {
...
export default function Home() {
  ...
  const [metricFocus, setMetricFocus] = useState<MetricFocus>("default");
  const [section, setSection] = useState<"dashboard" | "calendar" | "metrics" | "settings">("dashboard");
  ...
  useEffect(() => {
    ...
    if ("Notification" in window) {
      const enabled = Notification.permission === "granted" && savedReminders !== "false";
      setRemindersEnabled(enabled);
      setNotificationState(enabled ? "Notifications on" : Notification.permission === "denied" ? "Notifications blocked" : "Notifications off");
    } else setNotificationState("Not supported in this browser");
    setHydrated(true);
  }, []);
  ...
  const visible = useMemo(() => {
    if (filter === "Mine") {
      if (metricFocus === "mine-open") return mineOpen;
      if (metricFocus === "mine-overdue") return mineOverdue;
      if (metricFocus === "mine-resolved") return mineResolved;
      return mine;
    }
    if (filter === "Overdue") {
      if (metricFocus === "attention-oldest" || metricFocus === "attention-first") return attentionQueue.slice(0, 1);
      if (metricFocus === "attention-owners") return [...attentionQueue].sort((a, b) => a.owner.localeCompare(b.owner) || new Date(a.expected).getTime() - new Date(b.expected).getTime());
      return attentionQueue;
    }
    return issues;
  }, [issues, filter, metricFocus, personalOwner]);
  const dashboardView = filter === "Mine" ? "mine" : filter === "Overdue" ? "attention" : "overview";
  const pageTitle = section === "calendar" ? "Your work calendar ✦" : section === "metrics" ? "Signals & progress ✦" : section === "settings" ? "Settings ✦" : filter === "Mine" ? "My actions ✦" : filter === "Overdue" ? "Needs attention" : `Good afternoon, ${profile?.role || "there"} ✦`;
  const pageDescription = section === "calendar" ? "Choose a day to see every task and issue you logged." : section === "metrics" ? "A clear read on delivery pace, follow-through, and where to focus." : section === "settings" ? "Personalize your workspace, workflow, notifications, and local data." : filter === "Mine" ? "Your personal action list, separated from the wider team queue." : filter === "Overdue" ? "A focused triage view for work that has passed its expected update." : "A lovely little command center for keeping work moving.";
  ...
  async function toggleNotifications() {
    if (remindersEnabled) { setRemindersEnabled(false); setNotificationState("Notifications off"); return; }
    if (!("Notification" in window)) return setNotificationState("Not supported in this browser");
    const permission = Notification.permission === "granted" ? "granted" : await Notification.requestPermission();
    const enabled = permission === "granted";
    setRemindersEnabled(enabled);
    setNotificationState(enabled ? "Notifications on" : permission === "denied" ? "Notifications blocked" : "Notifications off");
  }
      ...
      {section === "settings" && <section className="settings-page" aria-labelledby="settings-page-title">
        <div className="settings-grid">
          ...
          <article className="settings-card">
            ...
            <p className="settings-copy">Get alerts for work that needs attention and your daily check-in.</p>
            <button className="settings-toggle" type="button" role="switch" aria-checked={remindersEnabled} onClick={toggleNotifications}><span><strong>Signal Petal notifications</strong><small>{notificationState}</small></span><span className={`switch-track ${remindersEnabled ? "is-on" : ""}`} aria-hidden="true"/></button>
          </article>
        </div>
