...
type StatusDraft = { id: string; name: string; original?: string; locked?: boolean };
type Issue = {
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const mergedStatuses = Array.from(new Set([...loadedStatuses, ...loadedIssues.map(issue => issue.status)].filter(status => status !== "Resolved")));
    setStatuses([...(mergedStatuses.length ? mergedStatuses : ["New"]), "Resolved"]);
  ...
  const mineOpen = mine.filter(i => i.status !== "Resolved");
  const mineOverdue = mine.filter(isOverdue);
  const mineResolved = mine.filter(i => i.status === "Resolved");
  ...
  function openStatusSettings() {
    setStatusDraft(statuses.map(name => ({ id: crypto.randomUUID(), name, original: name, locked: name === "Resolved" })));
    setStatusInput("");
    setStatusError("");
  ...
  function addStatus(event: FormEvent<HTMLFormElement>) {
    ...
    setStatusDraft(items => [...items.slice(0, -1), { id: crypto.randomUUID(), name }, items[items.length - 1]].filter(Boolean) as StatusDraft[]);
    setStatusInput("");
    setStatusError("");
  }
  function saveStatuses() {
    ...
    if (names.filter(name => name !== "Resolved").length === 0) return setStatusError("Keep at least one active status for new issues.");
    ...
    setIssues(items => items.map(issue => renamed.has(issue.status) ? { ...issue, status: renamed.get(issue.status) as Status } : issue));
    setStatuses(names);
  ...
  return <main className={`theme-${theme} ${darkMode ? "dark-mode" : ""}`}>
    ...
    {showStatusSettings && <div className="modal-backdrop" role="presentation" onMouseDown={e => { if (e.target === e.currentTarget) setShowStatusSettings(false); }}><section className="modal status-modal" role="dialog" aria-modal="true" aria-labelledby="status-settings-title"><button className="close" type="button" aria-label="Close status settings" onClick={() => setShowStatusSettings(false)}>×</button><p className="eyebrow">WORKFLOW SETTINGS</p><h2 id="status-settings-title">Customize statuses</h2><p className="modal-copy">Rename the stages you use, remove unused ones, or add your own. Resolved stays fixed so completion insights keep working.</p><div className="status-list">{statusDraft.map((item, index) => { const inUse = Boolean(item.original && issues.some(issue => issue.status === item.original)); return <div className="status-row" key={item.id}><input aria-label={`Status ${index + 1}`} value={item.name} disabled={item.locked} onChange={e => { const name = e.target.value; setStatusDraft(items => items.map(draft => draft.id === item.id ? { ...draft, name } : draft)); setStatusError(""); }}/>{item.locked ? <span className="status-lock">Required</span> : <button type="button" disabled={inUse} title={inUse ? "Rename this status instead—issues currently use it." : "Remove status"} onClick={() => setStatusDraft(items => items.filter(draft => draft.id !== item.id))}>Remove</button>}</div>; })}</div><form className="status-add" onSubmit={addStatus}><input value={statusInput} onChange={e => { setStatusInput(e.target.value); setStatusError(""); }} placeholder="Add a new status" aria-label="New status name"/><button className="secondary" type="submit">+ Add</button></form>{statusError && <p className="status-error" role="alert">{statusError}</p>}<div className="modal-actions"><button className="secondary" type="button" onClick={() => setShowStatusSettings(false)}>Cancel</button><button className="primary" type="button" onClick={saveStatuses}>Save statuses</button></div></section></div>}
