...
type SummaryBlock = { heading: string; items: string[] };
const renderBlocks = (blocks: SummaryBlock[]) => blocks.filter(block => block.items.length).map(block => [block.heading, ...block.items].filter(Boolean).join("\n")).join("\n\n");
const overdueSpan = (days: number) => `${days} day${days === 1 ? "" : "s"}`;
...
export default function Home() {
  ...
  function professionalSummary(withHeading: boolean) {
    ...
    const body = renderBlocks([
      { heading: `Delivered (${shipped.length})`, items: shipped.map(issue => `\u2022 ${issue.title}${issue.outcome.trim() ? ` \u2014 ${issue.outcome.trim()}` : ""}`) },
      { heading: "In progress", items: stalled.map(issue => `\u2022 ${issue.title} \u2014 ${overdueSpan(daysOverdue(issue))} past its due date${issue.followUpPeople.length ? `, with ${issue.followUpPeople.join(", ")}` : ""}`) },
      { heading: "", items: (shipped.length || logged.length) && !stalled.length ? ["Nothing is currently past its due date."] : [] },
      { heading: "Started this week", items: logged.map(issue => `\u2022 ${issue.title}`) },
      { heading: "Focus for next week", items: priorities.map(issue => `\u2022 ${issue.title} \u2014 ${issue.action.trim() || "next step still to be set"}`) },
      { heading: "Deliberately deferred", items: parked.map(issue => `\u2022 ${issue.title}`) },
    ]);
    const learnings = reviewTakeaway ? shipped.map(issue => issue.memory?.learning?.trim()).filter((text): text is string => Boolean(text)).slice(0, 2) : [];
