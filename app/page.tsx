...
export default function Home() {
  ...
  useEffect(() => { const saved = localStorage.getItem("signal-petal-issues"); if (saved) setIssues(JSON.parse(saved).map((i: Issue) => ({ ...i, createdAt: i.createdAt || i.updates?.[0]?.at || new Date().toISOString() }))); const savedProfile = localStorage.getItem("signal-petal-profile"); if (savedProfile) { try { const parsed = JSON.parse(savedProfile) as Profile; if (parsed.name?.trim() && parsed.role?.trim()) setProfile({ name: parsed.name.trim(), role: parsed.role.trim() }); } catch { localStorage.removeItem("signal-petal-profile"); } } setTheme(localStorage.getItem("signal-petal-theme") || "rose"); setDarkMode(localStorage.getItem("signal-petal-dark") === "true"); setHydrated(true); if ("Notification" in window) setNotificationState(Notification.permission === "granted" ? "Reminders on" : "Notifications off"); }, []);
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape") setShowDetail(false); };
